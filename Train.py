...
def main():
    ...
    L_seg = nn.CrossEntropyLoss(weight=torch.Tensor(class_weights))
    print("Starting training loop...")
    for batch_index, train_data in enumerate(tqdm(train_loader, desc="Training Batches")):
        ...
        if (batch_index+1) % 10 == 0: 
            print(f"[Train] Iter {batch_index+1}/{args.num_steps} | Time: {10*np.mean(hist[batch_index-9:batch_index+1,-1]):.2f}s | Batch_OA: {np.mean(hist[batch_index-9:batch_index+1,1])*100:.2f} | Batch_mIoU: {np.mean(hist[batch_index-9:batch_index+1,2])*100:.2f} | CE_loss: {np.mean(hist[batch_index-9:batch_index+1,0]):.3f}")
            f.write(f"[Train] Iter {batch_index+1}/{args.num_steps} | Time: {10*np.mean(hist[batch_index-9:batch_index+1,-1]):.2f}s | Batch_OA: {np.mean(hist[batch_index-9:batch_index+1,1])*100:.2f} | Batch_mIoU: {np.mean(hist[batch_index-9:batch_index+1,2])*100:.2f} | CE_loss: {np.mean(hist[batch_index-9:batch_index+1,0]):.3f}\n")
            f.flush() 
